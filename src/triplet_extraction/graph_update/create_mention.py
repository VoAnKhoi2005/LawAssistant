...
from src.triplet_extraction.graph_update.amendment_detection import (
    resolve_full_path,
...
def resolve_mention_reference(
    ...
    return resolve_full_path(sections_col, ref)
...
def main():
    ...
    for section in non_amendment_sections:
        ...
        refs = parse_amendment_reference(section['content'], documents_col)

        # Normalize refs → always List[Dict]
        if not refs:
            continue
            ...
            if target_node:
                ...
                relation = {
                    ...
                    "ref_details": {
                        ...
                        if k in [
                            'so_hieu', 'phan', 'chuong', 'muc',
                            'tieu_muc', 'dieu', 'khoan', 'diem', 'phu_luc'
                        ]
                    }
                }
    ...
    with open("mention_skipped.txt", "w", encoding="utf-8") as f:
        f.write("Mention references skipped or not found\n")
    ...
    print("\nSummary:")
    print(f"Total sections processed: {processed_sections}")
